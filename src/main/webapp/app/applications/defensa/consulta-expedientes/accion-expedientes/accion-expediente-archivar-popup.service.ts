...
@Injectable()
export class AccionExpedienteArchivarPopupService {
    ...
    open(component: Component, id?: number | any): Promise<NgbModalRef> {
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                ...
                // });
                setTimeout(() => {
                    this.ngbModalRef = this.accionExpedienteModalRef(component, new Expediente());
                    resolve(this.ngbModalRef);
                }, 0);
            } else {
    ...
    accionExpedienteModalRef(component: Component, expediente: Expediente): NgbModalRef {
        const modalRef = this.modalService.open(component, {  backdrop: 'static'});
        modalRef.componentInstance.expediente = expediente;
        modalRef.result.then((result) => {
            this.router.navigate(['defensa/expediente/accion'], { replaceUrl: true });
            this.ngbModalRef = null;
        }, (reason) => {
            this.router.navigate(['defensa/expediente/accion'], { replaceUrl: true });
            this.ngbModalRef = null;
