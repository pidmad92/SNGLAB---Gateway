...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.tipoEntidadService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/tipo-entidad'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: TipoEntidad) {
        return item.id;
    }
    registerChangeInTipoEntidads() {
        this.eventSubscriber = this.eventManager.subscribe('tipoEntidadListModification', (response) => this.loadAll());
    }
