...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.tipoUsuarioService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/tipo-usuario'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: TipoUsuario) {
        return item.id;
    }
    registerChangeInTipoUsuarios() {
        this.eventSubscriber = this.eventManager.subscribe('tipoUsuarioListModification', (response) => this.loadAll());
    }
